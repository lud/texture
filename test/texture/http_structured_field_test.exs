defmodule Texture.HttpStructuredFieldTest do
  ...
  describe "integers" do
    test "parses simple integer" do
      assert {:integer, 42, []} == ok!(HttpStructuredField.parse_item("42"))
    end

    test "parses integer 5" do
      assert {:integer, 5, []} == ok!(HttpStructuredField.parse_item("5"))
    end

    test "parses integer with parameters" do
      assert {:integer, 5, [{"foo", {:token, "bar"}}]} ==
               ok!(HttpStructuredField.parse_item("5; foo=bar"))
    end

    test "parses integer with boolean parameter" do
      assert {:integer, 1, [{"a", {:boolean, true}}, {"b", {:boolean, false}}]} ==
               ok!(HttpStructuredField.parse_item("1; a; b=?0"))
    end
    ...
    test "parses zero" do
      assert {:integer, 0, []} == ok!(HttpStructuredField.parse_item("0"))
    end
  ...
  describe "decimals" do
    test "parses simple decimal" do
      assert {:decimal, 4.5, []} == ok!(HttpStructuredField.parse_item("4.5"))
    end

    test "parses decimal with rating" do
      assert {:decimal, 1.5, []} == ok!(HttpStructuredField.parse_item("1.5"))
    end
    ...
    test "parses decimal with three fractional digits" do
      assert {:decimal, 123.456, []} == ok!(HttpStructuredField.parse_item("123.456"))
    end

    test "parses decimal with trailing zeros" do
      assert {:decimal, 5.230, []} == ok!(HttpStructuredField.parse_item("5.230"))
    end

    test "parses negative decimal with trailing zeros" do
      assert {:decimal, -0.40, []} == ok!(HttpStructuredField.parse_item("-0.40"))
    end

    test "parses zero decimal" do
      assert {:decimal, +0.0, []} == ok!(HttpStructuredField.parse_item("0.0"))
    end
    ...
    test "invalid decimal, two dots" do
      assert {:error, _} = HttpStructuredField.parse_item("1.234.5")
    end
  end
  ...
  describe "strings" do
    ...
    test "parses empty string" do
      assert {:string, "", []} == ok!(HttpStructuredField.parse_item(~S{""}))
    end

    test "parses string with escaped quotes" do
      assert {:string, "say \"hello\"", []} ==
               ok!(HttpStructuredField.parse_item(~S{"say \"hello\""}))
    end

    test "parses string with escaped backslash" do
      assert {:string, "path\\to\\file", []} ==
               ok!(HttpStructuredField.parse_item(~S{"path\\to\\file"}))
    end

    test "parses string Applepie" do
      assert {:string, "Applepie", []} ==
               ok!(HttpStructuredField.parse_item(~S{"Applepie"}))
    end

    test "parses string with URI" do
      assert {:string, "https://foo.example.com/", []} ==
               ok!(HttpStructuredField.parse_item(~S{"https://foo.example.com/"}))
    end

    test "invalid string, unfinished quote (unescaped quote)" do
      assert {:error, _} = HttpStructuredField.parse_item(~S{"hello world})
    end
  end

  describe "tokens" do
    test "parses simple token" do
      assert {:token, "sugar", []} == ok!(HttpStructuredField.parse_item("sugar"))
    end

    test "parses token tea" do
      assert {:token, "tea", []} == ok!(HttpStructuredField.parse_item("tea"))
    end

    test "parses token rum" do
      assert {:token, "rum", []} == ok!(HttpStructuredField.parse_item("rum"))
    end

    test "parses token with numbers and special chars" do
      assert {:token, "foo123/456", []} ==
               ok!(HttpStructuredField.parse_item("foo123/456"))
    end

    test "parses token abc" do
      assert {:token, "abc", []} == ok!(HttpStructuredField.parse_item("abc"))
    end

    test "parses token cde_456" do
      assert {:token, "cde_456", []} == ok!(HttpStructuredField.parse_item("cde_456"))
    end

    test "parses token ghi" do
      assert {:token, "ghi", []} == ok!(HttpStructuredField.parse_item("ghi"))
    end

    test "parses token l" do
      assert {:token, "l", []} == ok!(HttpStructuredField.parse_item("l"))
    end

    test "parses token w" do
      assert {:token, "w", []} == ok!(HttpStructuredField.parse_item("w"))
    end

    test "parses token joy" do
      assert {:token, "joy", []} == ok!(HttpStructuredField.parse_item("joy"))
    end

    test "parses token sadness" do
      assert {:token, "sadness", []} == ok!(HttpStructuredField.parse_item("sadness"))
    end

    test "parses token starting with asterisk" do
      assert {:token, "*foo", []} == ok!(HttpStructuredField.parse_item("*foo"))
    end

    test "invalid token (starts with digit)" do
      assert {:error, _} = HttpStructuredField.parse_item("123abc")
    end
  end
  ...
  describe "byte sequences" do
    test "parses simple byte sequence" do
      assert {:byte_sequence, "pretend this is binary content.", []} ==
               ok!(HttpStructuredField.parse_item(":cHJldGVuZCB0aGlzIGlzIGJpbmFyeSBjb250ZW50Lg==:"))
    end
    ...
    test "parses byte sequence w4ZibGV0w6ZydGU=" do
      ...
      assert {:byte_sequence, expected_bytes, []} ==
               ok!(HttpStructuredField.parse_item(":w4ZibGV0w6ZydGU=:"))
    end

    test "parses empty byte sequence" do
      assert {:byte_sequence, "", []} == ok!(HttpStructuredField.parse_item("::"))
    end

    test "invalid base64" do
      assert {:error, _} = HttpStructuredField.parse_item(":invalid@base64:")
    end
  end

  describe "booleans" do
    test "parses true boolean" do
      assert {:boolean, true, []} == ok!(HttpStructuredField.parse_item("?1"))
    end

    test "parses false boolean" do
      assert {:boolean, false, []} == ok!(HttpStructuredField.parse_item("?0"))
    end

    test "invalid boolean" do
      assert {:error, _} = HttpStructuredField.parse_item("?2")
    end
  end
  ...
  describe "items with parameters" do
    test "parses integer with string parameter" do
      assert {:integer, 2, [{"foourl", {:string, "https://foo.example.com/"}}]} ==
               ok!(HttpStructuredField.parse_item("2; foourl=\"https://foo.example.com/\""))
    end

    test "parses token with multiple parameters" do
      assert {:token, "abc", [{"a", {:integer, 1}}, {"b", {:integer, 2}}]} ==
               ok!(HttpStructuredField.parse_item("abc;a=1;b=2"))
    end
  ...
  describe "lists" do
    test "parses simple token list" do
      assert [
               ...
               {:token, "tea", []},
               {:token, "rum", []}
             ] == ok!(HttpStructuredField.parse_list("sugar, tea, rum"))
    end

    test "parses list with parameters" do
      assert [
               {:token, "abc", [{"a", {:integer, 1}}, {"b", {:integer, 2}}, {"cde_456", {:boolean, true}}]}
             ] == ok!(HttpStructuredField.parse_list("abc;a=1;b=2; cde_456"))
    end
    ...
    test "multiple items list with parameters" do
      assert [
               ...
               ok!(HttpStructuredField.parse_list(~S{abc;a=1;b=2; cde_456, (ghi;jk=4 l);q="9";r=w}))
    end
  ...
  describe "inner lists" do
    defp parse_inner_list!(input) do
      case HttpStructuredField.parse_list(input) do
        {:ok, [found]} ->
          found
  ...
  describe "lists with inner lists" do
    test "parses list of inner lists of strings" do
      assert [
               ...
               ok!(HttpStructuredField.parse_list(~S{("foo" "bar"), ("baz"), ("bat" "one"), ()}))
    end
    ...
    test "parses list of inner lists with parameters" do
      assert [
               ...
               ok!(HttpStructuredField.parse_list(~S{("foo"; a=1;b=2);lvl=5, ("bar" "baz");lvl=1}))
    end
    ...
    test "parses mixed list with tokens and inner lists" do
      assert [
             ...
             ] == ok!(HttpStructuredField.parse_list("(ghi;jk=4 l), l;q=\"9\";r=w"))
    end
  end
  ...
  describe "dictionaries" do
    test "parses simple dictionary" do
      assert [
               ...
               ok!(HttpStructuredField.parse_dict("en=\"Applepie\", da=:#{Base.encode64("hello")}:"))
    end
    ...
    test "parses dictionary with boolean values" do
      assert [
             ...
             ] == ok!(HttpStructuredField.parse_dict("a=?0, b, c; foo=bar"))
    end
    ...
    test "parses dictionary with decimal and inner list" do
      assert [
               ...
               ok!(HttpStructuredField.parse_dict("rating=1.5, feelings=(joy sadness)"))
    end
    ...
    test "parses dictionary with mix of items and inner lists" do
      assert [
               ...
               ok!(HttpStructuredField.parse_dict("a=(1 2), b=3, c=4;aa=bb, d=(5 6);valid"))
    end
    ...
    test "parses simple key-value dictionary" do
      assert [
               {"foo", {:integer, 1, []}},
               {"bar", {:integer, 2, []}}
             ] == ok!(HttpStructuredField.parse_dict("foo=1, bar=2"))
    end
  ...
  describe "edge cases for dicts" do
    test "dictionary with key= without value" do
      assert {:error, _} = HttpStructuredField.parse_dict("a=")
    end
    ...
    test "dictionary with very long key (edge case)" do
      ...
      assert [
               {long_key, {:integer, 1, []}}
             ] == ok!(HttpStructuredField.parse_dict("#{long_key}=1"))
    end
    ...
    test "dictionary with maximum items (edge case)" do
      ...
      result = ok!(HttpStructuredField.parse_dict(dict_string))
    ...
    test "dictionary with whitespace around delimiters" do
      assert [
               {"a", {:integer, 1, []}},
               {"b", {:integer, 2, []}}
             ] == ok!(HttpStructuredField.parse_dict("a=1 , b=2"))
    end

    test "dictionary with trailing comma (invalid)" do
      assert {:error, _} = HttpStructuredField.parse_dict("a=1, b=2,")
    end
    ...
    test "dictionary key with invalid characters" do
      assert {:error, _} = HttpStructuredField.parse_dict("UPPER=1")
      assert {:error, _} = HttpStructuredField.parse_dict("key with space=1")
      assert {:error, _} = HttpStructuredField.parse_dict("123key=1")
    end
    ...
    test "dictionary with complex nested parameters" do
      assert [
               ...
               ok!(HttpStructuredField.parse_dict("a=1;x;y=\"test\", b=(2);z=3.14"))
    end
  end
  ...
  describe "edge cases for integers" do
    test "integer with leading zeros" do
      ...
      assert {:integer, 42, []} == ok!(HttpStructuredField.parse_item("0042"))
      assert {:integer, -42, []} == ok!(HttpStructuredField.parse_item("-0042"))
    end
  ...
  describe "edge cases for decimals" do
    ...
    test "decimal ending with dot only (invalid)" do
      assert {:error, _} = HttpStructuredField.parse_item("123.")
    end

    test "decimal with multiple dots (invalid)" do
      assert {:error, _} = HttpStructuredField.parse_item("1.2.3")
    end
  end
  ...
  describe "edge cases for strings" do
    test "string with maximum length" do
      ...
      assert {:string, long_string, []} ==
               ok!(HttpStructuredField.parse_item(quoted_string))
    end

    test "string with all allowed escape sequences" do
      assert {:string, "quote: \" backslash: \\", []} ==
               ok!(HttpStructuredField.parse_item(~S{"quote: \" backslash: \\"}))
    end

    test "string with invalid escape sequences" do
      assert {:error, _} = HttpStructuredField.parse_item(~S{"invalid \x escape"})
      assert {:error, _} = HttpStructuredField.parse_item(~S{"invalid \n escape"})
      assert {:error, _} = HttpStructuredField.parse_item(~S{"invalid \t escape"})
    end
    ...
    test "string with control characters (invalid)" do
      ...
      assert {:error, _} = HttpStructuredField.parse_item("\"hello\x00world\"")
      assert {:error, _} = HttpStructuredField.parse_item("\"hello\x1fworld\"")
    end
  ...
  describe "edge cases for tokens" do
    test "token with maximum length" do
      ...
      assert {:token, long_token, []} == ok!(HttpStructuredField.parse_item(long_token))
    end

    test "token with all allowed characters" do
      # Test various tchar characters and ":" "/"
      assert {:token, "abc123_-.*:/%", []} ==
               ok!(HttpStructuredField.parse_item("abc123_-.*:/%"))
    end

    test "token starting with invalid characters" do
      assert {:error, _} = HttpStructuredField.parse_item("123invalid")
      assert {:error, _} = HttpStructuredField.parse_item("-invalid")
      assert {:error, _} = HttpStructuredField.parse_item(".invalid")
    end
    ...
    test "token with uppercase letters (invalid in some contexts)" do
      ...
      assert {:token, "ValidToken", []} ==
               ok!(HttpStructuredField.parse_item("ValidToken"))
    end
  end
  ...
  describe "edge cases for byte sequences" do
    test "byte sequence with maximum length" do
      ...
      assert {:byte_sequence, large_data, []} ==
               ok!(HttpStructuredField.parse_item(":#{encoded}:"))
    end
    ...
    test "byte sequence with padding variations" do
      ...
      assert {:byte_sequence, "sure.", []} ==
               ok!(HttpStructuredField.parse_item(":c3VyZS4=:"))
    end

    test "byte sequence with invalid base64 characters" do
      assert {:error, _} = HttpStructuredField.parse_item(":invalid@chars:")
      assert {:error, _} = HttpStructuredField.parse_item(":contains spaces:")
    end

    test "byte sequence missing closing colon" do
      assert {:error, _} = HttpStructuredField.parse_item(":dGVzdA==")
    end
  ...
  describe "edge cases for booleans" do
    test "invalid boolean values" do
      assert {:error, _} = HttpStructuredField.parse_item("?2")
      assert {:error, _} = HttpStructuredField.parse_item("?true")
      assert {:error, _} = HttpStructuredField.parse_item("?false")
      assert {:error, _} = HttpStructuredField.parse_item("??1")
  ...
  describe "edge cases for parameters" do
    test "parameter with maximum count" do
      ...
      assert {:token, "token", parameters} = ok!(HttpStructuredField.parse_item(input))
    ...
    test "parameter key with maximum length" do
      ...
      assert {:integer, 1, [{long_key, {:integer, 2}}]} ==
               ok!(HttpStructuredField.parse_item("1;#{long_key}=2"))
    end
    ...
    test "parameter key with invalid characters" do
      ...
      assert {:error, _} = HttpStructuredField.parse_item("1;-invalid=2")
    end

    test "parameter without value (boolean true)" do
      assert {:integer, 1, [{"flag", {:boolean, true}}]} ==
               ok!(HttpStructuredField.parse_item("1;flag"))
    end

    test "parameter with empty key" do
      assert {:error, _} = HttpStructuredField.parse_item("1;=value")
    end
  end
  ...
  describe "edge cases for lists" do
    test "list with maximum members" do
      ...
      result = ok!(HttpStructuredField.parse_list(list_string))
      assert 100 == length(result)
    end

    test "list with trailing comma (invalid)" do
      assert {:error, _} = HttpStructuredField.parse_list("1, 2, 3,")
    end

    test "list with consecutive commas (invalid)" do
      assert {:error, _} = HttpStructuredField.parse_list("1,, 2")
    end
  ...
  describe "edge cases for inner lists" do
    ...
    test "nested inner lists (invalid)" do
      ...
      assert {:error, _} = HttpStructuredField.parse_list("((1))")
    end
  end
  ...
  describe "whitespace and formatting edge cases" do
    ...
    test "tab characters in various positions" do
      ...
      assert [
               {"a", {:integer, 1, []}}
             ] == ok!(HttpStructuredField.parse_dict("\t\ta=1\t"))
    end

    test "mixed whitespace types" do
      assert {:integer, 1, [{"a", {:integer, 2}}]} ==
               ok!(HttpStructuredField.parse_item(" \t  1 ;\t a=2 \t \t "))
    end
  end
  ...
  describe "malformed input edge cases" do
    test "completely invalid input" do
      assert {:error, _} = HttpStructuredField.parse_item("@#$%&*()")
      assert {:error, _} = HttpStructuredField.parse_list("@#$%")
      assert {:error, _} = HttpStructuredField.parse_dict("@#$%")
    end

    test "partial valid input" do
      # Valid start but becomes invalid
      assert {:error, _} = HttpStructuredField.parse_item("123abc@#$")
      assert {:error, _} = HttpStructuredField.parse_item("\"hello@#$")
    end

    test "unicode characters (invalid)" do
      # RFC 8941: Strings are ASCII only
      assert {:error, _} = HttpStructuredField.parse_item("\"héllo\"")
      assert {:error, _} = HttpStructuredField.parse_item("tökën")
    end
  end
  ...
  describe "with unwrap: true option" do
    test "unwrapped integers" do
      ...
      assert {-999_999_999_999_999, []} ==
               ok!(HttpStructuredField.parse_item("-999999999999999", unwrap: true))

      assert {5, [{"foo", "bar"}]} ==
               ok!(HttpStructuredField.parse_item("5; foo=bar", unwrap: true))
    end

    test "unwrapped decimals" do
      assert {4.5, []} == ok!(HttpStructuredField.parse_item("4.5", unwrap: true))
      assert {-1.5, []} == ok!(HttpStructuredField.parse_item("-1.5", unwrap: true))

      assert {123.456, [{"precision", 3}]} ==
               ok!(HttpStructuredField.parse_item("123.456; precision=3", unwrap: true))
    end

    test "unwrapped strings" do
      assert {"hello world", []} ==
               ok!(HttpStructuredField.parse_item(~S{"hello world"}, unwrap: true))

      assert {"", []} == ok!(HttpStructuredField.parse_item(~S{""}, unwrap: true))

      assert {"Applepie", [{"lang", "en"}]} ==
               ok!(HttpStructuredField.parse_item(~S{"Applepie"; lang=en}, unwrap: true))
    end

    test "unwrapped tokens" do
      assert {"sugar", []} == ok!(HttpStructuredField.parse_item("sugar", unwrap: true))

      assert {"foo123/456", []} ==
               ok!(HttpStructuredField.parse_item("foo123/456", unwrap: true))

      assert {"abc", [{"a", 1}, {"b", 2}]} ==
               ok!(HttpStructuredField.parse_item("abc;a=1;b=2", unwrap: true))
    end

    test "unwrapped byte sequences" do
      assert {"pretend this is binary content.", []} ==
               ok!(
                 HttpStructuredField.parse_item(
                   ":cHJldGVuZCB0aGlzIGlzIGJpbmFyeSBjb250ZW50Lg==:",
                   unwrap: true
                 )
               )

      assert {"", []} == ok!(HttpStructuredField.parse_item("::", unwrap: true))
      expected_bytes = Base.decode64!("w4ZibGV0w6ZydGU=")

      assert {expected_bytes, [{"encoding", "utf8"}]} ==
               ok!(
                 HttpStructuredField.parse_item(":w4ZibGV0w6ZydGU=:; encoding=utf8",
                   unwrap: true
                 )
               )
    end
    ...
    test "unwrapped booleans" do
      assert {true, []} == ok!(HttpStructuredField.parse_item("?1", unwrap: true))
      assert {false, []} == ok!(HttpStructuredField.parse_item("?0", unwrap: true))
    ...
    test "unwrapped inner lists" do
      assert [{[{"foo", []}, {"bar", []}], []}] ==
               ok!(HttpStructuredField.parse_list(~S{("foo" "bar")}, unwrap: true))

      assert [{[], []}] == ok!(HttpStructuredField.parse_list("()", unwrap: true))

      assert [{[{1, [{"a", 2}]}, {3, []}], [{"lvl", 5}]}] ==
               ok!(HttpStructuredField.parse_list("(1;a=2 3);lvl=5", unwrap: true))
    end

    test "unwrapped lists" do
      assert [{"sugar", []}, {"tea", []}, {"rum", []}] ==
               ok!(HttpStructuredField.parse_list("sugar, tea, rum", unwrap: true))

      assert [{"abc", [{"a", 1}]}] ==
               ok!(HttpStructuredField.parse_list("abc;a=1", unwrap: true))

      assert [{"abc", [{"flag", true}]}, {[{"ghi", []}, {"l", []}], [{"q", "9"}]}] ==
               ok!(HttpStructuredField.parse_list(~S{abc;flag, (ghi l);q="9"}, unwrap: true))
    end

    test "unwrapped dictionaries" do
      assert [{"foo", {1, []}}, {"bar", {2, []}}] ==
               ok!(HttpStructuredField.parse_dict("foo=1, bar=2", unwrap: true))

      assert [{"a", {false, []}}, {"b", {true, []}}] ==
               ok!(HttpStructuredField.parse_dict("a=?0, b", unwrap: true))

      assert [{"rating", {1.5, []}}, {"feelings", {[{"joy", []}, {"sadness", []}], []}}] ==
               ok!(
                 HttpStructuredField.parse_dict("rating=1.5, feelings=(joy sadness)",
                   unwrap: true
                 )
               )
    end
  ...
  describe "with maps: true option" do
    test "integers with maps for parameters and dictionaries" do
      assert {:integer, 42, %{}} == ok!(HttpStructuredField.parse_item("42", maps: true))

      assert {:integer, -999_999_999_999_999, %{}} ==
               ok!(HttpStructuredField.parse_item("-999999999999999", maps: true))

      assert {:integer, 5, %{"foo" => {:token, "bar"}}} ==
               ok!(HttpStructuredField.parse_item("5; foo=bar", maps: true))
    end

    test "decimals with maps for parameters and dictionaries" do
      assert {:decimal, 4.5, %{}} == ok!(HttpStructuredField.parse_item("4.5", maps: true))

      assert {:decimal, -1.5, %{}} ==
               ok!(HttpStructuredField.parse_item("-1.5", maps: true))

      assert {:decimal, 123.456, %{"precision" => {:integer, 3}}} ==
               ok!(HttpStructuredField.parse_item("123.456; precision=3", maps: true))
    end

    test "strings with maps for parameters and dictionaries" do
      assert {:string, "hello world", %{}} ==
               ok!(HttpStructuredField.parse_item(~S{"hello world"}, maps: true))

      assert {:string, "", %{}} == ok!(HttpStructuredField.parse_item(~S{""}, maps: true))

      assert {:string, "Applepie", %{"lang" => {:token, "en"}}} ==
               ok!(HttpStructuredField.parse_item(~S{"Applepie"; lang=en}, maps: true))
    end

    test "tokens with maps for parameters and dictionaries" do
      assert {:token, "sugar", %{}} ==
               ok!(HttpStructuredField.parse_item("sugar", maps: true))

      assert {:token, "foo123/456", %{}} ==
               ok!(HttpStructuredField.parse_item("foo123/456", maps: true))

      assert {:token, "abc", %{"a" => {:integer, 1}, "b" => {:integer, 2}}} ==
               ok!(HttpStructuredField.parse_item("abc;a=1;b=2", maps: true))
    end
    ...
    test "byte sequences with maps for parameters and dictionaries" do
      ...
      assert {:byte_sequence, "", %{}} ==
               ok!(HttpStructuredField.parse_item("::", maps: true))
    ...
    test "booleans with maps for parameters and dictionaries" do
      assert {:boolean, true, %{}} == ok!(HttpStructuredField.parse_item("?1", maps: true))

      assert {:boolean, false, %{}} ==
               ok!(HttpStructuredField.parse_item("?0", maps: true))
    ...
    test "inner lists with maps for parameters and dictionaries" do
      assert [{:inner_list, [{:string, "foo", %{}}, {:string, "bar", %{}}], %{}}] ==
               ok!(HttpStructuredField.parse_list(~S{("foo" "bar")}, maps: true))

      assert [{:inner_list, [], %{}}] ==
               ok!(HttpStructuredField.parse_list("()", maps: true))

      assert [
               {:inner_list, [{:integer, 1, %{"a" => {:integer, 2}}}, {:integer, 3, %{}}], %{"lvl" => {:integer, 5}}}
             ] ==
               ok!(HttpStructuredField.parse_list("(1;a=2 3);lvl=5", maps: true))
    end
    ...
    test "lists with maps for parameters and dictionaries" do
      assert [
               ...
               {:token, "tea", %{}},
               {:token, "rum", %{}}
             ] == ok!(HttpStructuredField.parse_list("sugar, tea, rum", maps: true))

      assert [
               {:token, "abc", %{"a" => {:integer, 1}}}
             ] == ok!(HttpStructuredField.parse_list("abc;a=1", maps: true))

      assert [
               {:token, "abc", %{"flag" => {:boolean, true}}},
               {:inner_list, [{:token, "ghi", %{}}, {:token, "l", %{}}], %{"q" => {:string, "9"}}}
             ] ==
               ok!(HttpStructuredField.parse_list(~S{abc;flag, (ghi l);q="9"}, maps: true))
    end
    ...
    test "dictionaries with maps for parameters and dictionaries" do
      assert %{
               "foo" => {:integer, 1, %{}},
               "bar" => {:integer, 2, %{}}
             } == ok!(HttpStructuredField.parse_dict("foo=1, bar=2", maps: true))

      assert %{
               "a" => {:boolean, false, %{}},
               "b" => {:boolean, true, %{}}
             } == ok!(HttpStructuredField.parse_dict("a=?0, b", maps: true))
  ...
  describe "with both unwrap: true and maps: true options" do
    test "integers without type tags and with maps" do
      assert {42, %{}} ==
               ok!(HttpStructuredField.parse_item("42", unwrap: true, maps: true))

      assert {-42, %{}} ==
               ok!(HttpStructuredField.parse_item("-42", unwrap: true, maps: true))

      assert {5, %{"foo" => "bar"}} ==
               ok!(HttpStructuredField.parse_item("5; foo=bar", unwrap: true, maps: true))
    end

    test "decimals without type tags and with maps" do
      assert {4.5, %{}} ==
               ok!(HttpStructuredField.parse_item("4.5", unwrap: true, maps: true))

      assert {-1.5, %{}} ==
               ok!(HttpStructuredField.parse_item("-1.5", unwrap: true, maps: true))

      assert {123.456, %{"precision" => 3}} ==
               ok!(
                 HttpStructuredField.parse_item("123.456; precision=3",
                   unwrap: true,
                   maps: true
                 )
               )
    ...
    test "strings without type tags and with maps" do
      assert {"hello world", %{}} ==
               ok!(HttpStructuredField.parse_item(~S{"hello world"}, unwrap: true, maps: true))

      assert {"", %{}} ==
               ok!(HttpStructuredField.parse_item(~S{""}, unwrap: true, maps: true))

      assert {"Applepie", %{"lang" => "en"}} ==
               ok!(
                 HttpStructuredField.parse_item(~S{"Applepie"; lang=en},
                   unwrap: true,
                   maps: true
                 )
               )
    ...
    test "tokens without type tags and with maps" do
      assert {"sugar", %{}} ==
               ok!(HttpStructuredField.parse_item("sugar", unwrap: true, maps: true))

      assert {"foo123/456", %{}} ==
               ok!(HttpStructuredField.parse_item("foo123/456", unwrap: true, maps: true))

      assert {"abc", %{"a" => 1, "b" => 2}} ==
               ok!(HttpStructuredField.parse_item("abc;a=1;b=2", unwrap: true, maps: true))
    end
    ...
    test "byte sequences without type tags and with maps" do
      ...
      assert {"", %{}} ==
               ok!(HttpStructuredField.parse_item("::", unwrap: true, maps: true))
    ...
    test "booleans without type tags and with maps" do
      assert {true, %{}} ==
               ok!(HttpStructuredField.parse_item("?1", unwrap: true, maps: true))

      assert {false, %{}} ==
               ok!(HttpStructuredField.parse_item("?0", unwrap: true, maps: true))

      assert {true, %{"flag" => "active"}} ==
               ok!(HttpStructuredField.parse_item("?1; flag=active", unwrap: true, maps: true))
    end

    test "inner lists without type tags and with maps" do
      assert [{[{"foo", %{}}, {"bar", %{}}], %{}}] ==
               ok!(HttpStructuredField.parse_list(~S{("foo" "bar")}, unwrap: true, maps: true))

      assert [{[], %{}}] ==
               ok!(HttpStructuredField.parse_list("()", unwrap: true, maps: true))

      assert [{[{1, %{"a" => 2}}, {3, %{}}], %{"lvl" => 5}}] ==
               ok!(HttpStructuredField.parse_list("(1;a=2 3);lvl=5", unwrap: true, maps: true))
    end

    test "lists without type tags and with maps" do
      assert [{"sugar", %{}}, {"tea", %{}}, {"rum", %{}}] ==
               ok!(HttpStructuredField.parse_list("sugar, tea, rum", unwrap: true, maps: true))

      assert [{"abc", %{"a" => 1, "b" => 2, "cde_456" => true}}] ==
               ok!(
                 HttpStructuredField.parse_list("abc;a=1;b=2; cde_456",
                   unwrap: true,
                   maps: true
                 )
               )
    ...
    test "dictionaries without type tags and with maps" do
      assert %{
               ...
               ok!(HttpStructuredField.parse_dict("foo=1, bar=2", unwrap: true, maps: true))

      assert %{
               "a" => {false, %{}},
               "b" => {true, %{}}
             } ==
               ok!(HttpStructuredField.parse_dict("a=?0, b", unwrap: true, maps: true))
